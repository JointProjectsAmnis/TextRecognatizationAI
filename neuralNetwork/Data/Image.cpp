...
Image::Image(double* image, int sizeX, int sizeY, int channel, bool inHost)
{
...
* типом double, а также нормализуетс€ в границах от 0 до 1.
*/
void Image::loadImage(const char* path, int channel)
{
	...
	double* image = new double[sizeX * sizeY * this->channel];
	for (int x = 0; x < sizeX * sizeY * this->channel; x++)
		image[x] = chImage[x] / 255.0f;

	delete[sizeX * sizeY * this->channel] chImage;

	cudaError_t error = cudaMalloc(&dev_image, sizeof(double) * sizeX * sizeY * this->channel);
	if (error != 0) throw;

	error = cudaMemcpy(dev_image, image, sizeof(double) * sizeX * sizeY * this->channel, cudaMemcpyHostToDevice);
	if (error != 0) throw;

	delete[sizeX * sizeY * this->channel] image;
}

void Image::loadImage(double* image, int sizeX, int sizeY, int channel, bool inHost)
{
	if (dev_image != nullptr)
		freeMemory();
	if (inHost)
	{
		...
		cudaError_t error = cudaMalloc(&dev_image, sizeof(double) * sizeX * sizeY * channel);
		if (error != 0) throw;

		error = cudaMemcpy(dev_image, image, sizeof(double) * sizeX * sizeY * channel, cudaMemcpyHostToDevice);
		if (error != 0) throw;
	}
	else
	{
...
void Image::loadImage(unsigned char* image, int sizeX, int sizeY, int channel)
{
	...
	double* fimage = new double[sizeX * sizeY * channel];
	...
	error = cudaMemcpy(dev_image, fimage, sizeof(double) * sizeX * sizeY * channel, cudaMemcpyHostToDevice);
	if (error != 0) throw;

	delete[sizeX * sizeY * channel] fimage;
}

void Image::getImage(double*& outImage)
{
	if (outImage == nullptr)
		outImage = new double[sizeX * sizeY * channel];
	cudaError_t error = cudaMemcpy(outImage, dev_image, sizeof(double) * sizeX * sizeY * channel, cudaMemcpyDeviceToHost);
	if (error != 0) throw;
}

void Image::getImage(unsigned char*& outImage)
{
	double* fImage = nullptr;
	getImage(fImage);
	flToCh(fImage, outImage, sizeX, sizeY, channel);
	delete[sizeX * sizeY * channel] fImage;
}

void Image::flToCh(double* fimage, unsigned char*& outChImage, int sizeX, int sizeY, int channel)
{
...
void Image::chToFl(unsigned char* chImage, double*& outFImage, int sizeX, int sizeY, int channel)
{
